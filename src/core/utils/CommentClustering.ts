...
const GENERIC_ASPECT_TOKENS = new Set([
  'creator', 'male', 'female', 'host', 'video', 'person', 'people', 'shows', 'show',
  'demonstrates', 'demonstrate', 'demonstrating', 'explains', 'explaining', 'discusses',
  'discuss', 'presents', 'using', 'used', 'ready', 'tips', 'tip', 'claims', 'claim',
  'reveals', 'reveal', 'suggests', 'suggest', 'recommends', 'recommend', 'concludes',
  'conclude', 'takes', 'take', 'perfectly'
]);
...
export class CommentClusteringService {
  ...
  private async buildLocalClusters(
    ...
    const mergedClusters = this.mergeRedundantLocalClusters(videoId, localClusters, opinionUnits, enrichedByCommentId, logs);

    return this.normalizeLocalClusterImportance(mergedClusters)
  ...
  private buildInsightsFromLocalClusters(videoId: string, localClusters: LocalCluster[]): Insight[] {
    const hasSpecificGroundedCluster = localClusters.some(cluster =>
      cluster.primaryAspectKey !== 'video.video_wide_discussion' &&
      cluster.averageGroundingConfidence >= 0.24
    );

    const seenThemes = new Set<string>();
    const selectedClusters = localClusters
      .filter(cluster => this.isInsightCandidate(cluster, hasSpecificGroundedCluster))
      .sort((left, right) => this.scoreInsightCandidate(right) - this.scoreInsightCandidate(left))
      .filter(cluster => {
        const themeKey = this.buildInsightThemeKey(cluster);
  ...
  private async buildMetaClusters(localClusters: LocalCluster[], allVideoIds: string[], logs: string[]): Promise<MetaCluster[]> {
    ...
    const eligibleClusters = localClusters
      .filter(cluster => this.isMetaEligibleCluster(cluster))
      .sort((left, right) => right.importanceScore - left.importanceScore);
    ...
    for (const cluster of eligibleClusters) {
      ...
      for (let index = 0; index < accumulators.length; index += 1) {
        ...
        if (!this.areAspectsCompatible(cluster, candidate.localClusters[0])) {
          continue;
        }
    ...
    const mergedAccumulators = this.mergeRedundantMetaAccumulators(accumulators, logs);
    logs.push(`Created ${mergedAccumulators.length} meta-cluster candidates`);

    const metaClusters = mergedAccumulators
  ...
  private buildInsightsFromMetaClusters(metaClusters: MetaCluster[]): Insight[] {
    ...
    for (const cluster of metaClusters) {
      if (seenTitles.has(cluster.label)) {
        continue;
      }

      seenTitles.add(cluster.label);
      insights.push({
        insightId: `cross_video_insight_${insights.length + 1}`,
        scope: 'cross_video',
  ...
  private buildSingleVideoSummary(localClusters: LocalCluster[], insights: Insight[]): string {
    ...
    return [
      ...
      `Top theme: ${(topInsight?.title || localClusters[0].label)}.`,
  ...
  private mergeRedundantLocalClusters(
    videoId: string,
    ...
    const unitsById = new Map(opinionUnits.map(unit => [unit.opinionUnitId, unit]));
    const merged: LocalCluster[] = [];

    for (const cluster of clusters) {
      const existingIndex = merged.findIndex(candidate =>
        candidate.clusterType === cluster.clusterType &&
        candidate.stanceProfile.dominant === cluster.stanceProfile.dominant &&
        this.areLocalThemesEquivalent(candidate, cluster)
      );
      ...
      const combinedUnits = Array.from(new Set([...existing.unitIds, ...cluster.unitIds]))
        .map(unitId => unitsById.get(unitId))
        .filter((unit): unit is OpinionUnit => Boolean(unit));

      // Merge duplicate same-aspect clusters so short segments or minor lexical
      // differences do not create repeated local themes inside one video. When
      // an entity-level bucket and a controversy-level bucket clearly refer to
      // the same topic, keep the more specific aspect label.
      const preferredAspect = this.choosePreferredAspectCluster(existing, cluster);
      merged[existingIndex] = this.finalizeLocalCluster(
        videoId,
        preferredAspect.primaryAspectKey,
        preferredAspect.primaryAspectLabel,
        Array.from(new Set([...existing.anchorIds, ...cluster.anchorIds])),
        combinedUnits,
        enrichedByCommentId
      );
    }

    if (merged.length !== clusters.length) {
      logs.push(`Merged ${clusters.length - merged.length} redundant local clusters for ${videoId}`);
    }

    return merged;
  }

  private isMetaEligibleCluster(cluster: LocalCluster): boolean {
    if (cluster.clusterType === 'spam') return false;
    if (cluster.clusterType === 'reaction_only') return false;
    if (cluster.primaryAspectKey === 'video.video_wide_discussion') return false;
    if (cluster.primaryAspectKey.startsWith('visual_text.') && countMeaningfulTokens(cluster.primaryAspectLabel) < 2) {
      return false;
    }
    if (cluster.primaryAspectLabel === 'Video-wide discussion' && cluster.size < 2) {
      return false;
    }
  ...
  private isInsightCandidate(cluster: LocalCluster, preferSpecificGroundedClusters: boolean): boolean {
    if (cluster.clusterType === 'spam' || cluster.clusterType === 'reaction_only') {
      return false;
    }

    const broadScopeCluster =
      cluster.primaryAspectKey === 'video.video_wide_discussion' ||
      cluster.primaryAspectKey.startsWith('global_video.');
    const aspectFamily = cluster.primaryAspectKey.split('.')[0];

    if (preferSpecificGroundedClusters && broadScopeCluster) {
      return false;
    }
    ...
    // Singleton entity clusters are often useful as evidence, but they are too
    // brittle to headline the analysis when stronger grounded themes exist.
    if (
      preferSpecificGroundedClusters &&
      aspectFamily === 'entity' &&
      cluster.size < 2 &&
    ...
    if (
      aspectFamily === 'visual_text' &&
      cluster.size < 2 &&
      cluster.averageGroundingConfidence < 0.35
    ) {
      return false;
    }
    ...
    if (
      cluster.averageGroundingConfidence < 0.24 &&
      cluster.averageSemanticDensity < 0.45 &&
      cluster.size < 2 &&
      cluster.clusterType !== 'question'
    ) {
      return false;
  ...
  private scoreInsightCandidate(cluster: LocalCluster): number {
    const coverageScore = Math.min(1, cluster.size / 3);
    const aspectFamily = cluster.primaryAspectKey.split('.')[0];
    let specificityPenalty = 1;

    if (cluster.primaryAspectKey === 'video.video_wide_discussion') {
      specificityPenalty = 0.08;
    } else if (cluster.primaryAspectKey.startsWith('global_video.')) {
      specificityPenalty = 0.16;
    } else if (aspectFamily === 'entity' && cluster.size < 2) {
      specificityPenalty = 0.42;
    } else if (aspectFamily === 'visual_text' && cluster.size < 2) {
      specificityPenalty = 0.35;
    }

    return (
      cluster.importanceScore * 0.45 +
      cluster.averageGroundingConfidence * 0.25 +
      cluster.averageSemanticDensity * 0.15 +
      coverageScore * 0.15
    ) * specificityPenalty;
  }

  private buildInsightThemeKey(cluster: LocalCluster): string {
    const normalizedTokens = uniqueTokens(cluster.primaryAspectLabel)
      .filter(token => !GENERIC_ASPECT_TOKENS.has(token))
      .slice(0, 4)
      .sort();
    return normalizedTokens.join('_') || cluster.primaryAspectKey;
  }

  private areLocalThemesEquivalent(left: LocalCluster, right: LocalCluster): boolean {
    if (left.primaryAspectKey === right.primaryAspectKey) {
      return true;
    }

    if (!this.areAspectsCompatible(left, right)) {
      return false;
    }

    const sharedMeaningfulTokens = this.countSharedMeaningfulAspectTokens(
      left.primaryAspectLabel,
      right.primaryAspectLabel
    );
    const leftFamily = left.primaryAspectKey.split('.')[0];
    const rightFamily = right.primaryAspectKey.split('.')[0];

    if ((leftFamily === 'entity' || rightFamily === 'entity') && sharedMeaningfulTokens >= 1) {
      return true;
    }
  ...
  private choosePreferredAspectCluster(left: LocalCluster, right: LocalCluster): LocalCluster {
    const leftPriority = this.clusterAspectPriority(left.primaryAspectKey);
    const rightPriority = this.clusterAspectPriority(right.primaryAspectKey);
    ...
    return right.averageGroundingConfidence > left.averageGroundingConfidence ? right : left;
  }

  private clusterAspectPriority(aspectKey: string): number {
    const family = aspectKey.split('.')[0];
    switch (family) {
      case 'controversy':
        return 5;
      case 'moment':
  ...
  private areAspectsCompatible(left: LocalCluster, right: LocalCluster): boolean {
    if (left.primaryAspectKey === right.primaryAspectKey) {
      return true;
    }
    ...
    const leftFamily = left.primaryAspectKey.split('.')[0];
    const rightFamily = right.primaryAspectKey.split('.')[0];
    const labelSimilarity = jaccardSimilarity(left.primaryAspectLabel, right.primaryAspectLabel);
    const clusterTextSimilarity = jaccardSimilarity(
      `${left.label} ${left.summary}`,
      `${right.label} ${right.summary}`
    );
    const sharedMeaningfulTokens = this.countSharedMeaningfulAspectTokens(
    ...
    // Generic video-wide discussion clusters are only comparable to each other.
    if (left.primaryAspectKey === 'video.video_wide_discussion' || right.primaryAspectKey === 'video.video_wide_discussion') {
      return left.primaryAspectKey === right.primaryAspectKey;
    }

    if (leftFamily === 'entity' && rightFamily === 'entity') {
      return labelSimilarity >= 0.3;
    }

    if (
      ['moment', 'timeline_segment', 'controversy', 'spoken_quote'].includes(leftFamily) &&
      ['moment', 'timeline_segment', 'controversy', 'spoken_quote'].includes(rightFamily)
    ) {
      return (
        (sharedMeaningfulTokens >= 2 && (labelSimilarity >= 0.08 || clusterTextSimilarity >= 0.22)) ||
        labelSimilarity >= 0.18 ||
        clusterTextSimilarity >= 0.3
      );
    }

    if (
      (leftFamily === 'entity' && ['moment', 'timeline_segment', 'controversy', 'spoken_quote'].includes(rightFamily)) ||
      (rightFamily === 'entity' && ['moment', 'timeline_segment', 'controversy', 'spoken_quote'].includes(leftFamily))
    ) {
      return (
        this.labelsShareSpecificPhrase(left.primaryAspectLabel, right.primaryAspectLabel) ||
        (sharedMeaningfulTokens >= 2 && clusterTextSimilarity >= 0.16)
      );
    }

    // Visual-text based clusters should only merge when the visual phrase itself
    // is genuinely the same phrase, not just because generic wording looks close.
    if (leftFamily === 'visual_text' || rightFamily === 'visual_text') {
      return labelSimilarity >= 0.45;
    }

    if (leftFamily === rightFamily) {
      return (
        labelSimilarity >= 0.12 ||
        clusterTextSimilarity >= 0.22 ||
        (sharedMeaningfulTokens >= 1 && clusterTextSimilarity >= 0.18)
      );
    }

    return sharedMeaningfulTokens >= 1 && labelSimilarity >= 0.2 && clusterTextSimilarity >= 0.28;
  }

  private mergeRedundantMetaAccumulators(
    accumulators: MetaClusterAccumulator[],
    logs: string[]
  ): MetaClusterAccumulator[] {
    const merged: MetaClusterAccumulator[] = [];

    for (const accumulator of accumulators) {
      const existingIndex = merged.findIndex(candidate => {
        const left = accumulator.localClusters[0];
        const right = candidate.localClusters[0];
        return (
          this.areAspectsCompatible(left, right) &&
          this.areStancesCompatible(left.stanceProfile.dominant, right.stanceProfile.dominant) &&
          jaccardSimilarity(accumulator.signatureText, candidate.signatureText) >= 0.24
        );
      });
      ...
      const existing = merged[existingIndex];
      existing.localClusters.push(...accumulator.localClusters);
      existing.signatureText = `${existing.signatureText} ${accumulator.signatureText}`;
    }

    if (merged.length !== accumulators.length) {
      logs.push(`Merged ${accumulators.length - merged.length} redundant meta-cluster candidates`);
    }
  ...
  private countSharedMeaningfulAspectTokens(left: string, right: string): number {
    const leftTokens = uniqueTokens(left).filter(token => !GENERIC_ASPECT_TOKENS.has(token));
    const rightTokens = new Set(uniqueTokens(right).filter(token => !GENERIC_ASPECT_TOKENS.has(token)));
    return leftTokens.filter(token => rightTokens.has(token)).length;
  }

  private labelsShareSpecificPhrase(left: string, right: string): boolean {
    const leftTokens = uniqueTokens(left).filter(token => !GENERIC_ASPECT_TOKENS.has(token));
    const rightTokens = uniqueTokens(right).filter(token => !GENERIC_ASPECT_TOKENS.has(token));
    const leftPhrase = leftTokens.join(' ').trim();
    const rightPhrase = rightTokens.join(' ').trim();

    if (leftPhrase.length >= 6 && right.toLowerCase().includes(leftPhrase)) {
      return true;
    }

    if (rightPhrase.length >= 6 && left.toLowerCase().includes(rightPhrase)) {
      return true;
    }
