...
export class CommentClusteringService {
   ...
   */
  private async performClustering(comments: Comment[], embeddings: number[][], logs: string[]): Promise<CommentCluster[]> {
   ...
   */
  private async analyzeClusterThemes(clusters: CommentCluster[], logs: string[]): Promise<string> {
    try {
      // Import OpenAI dynamically
      const { OpenAI } = await import('openai');
      const openai = new OpenAI({ apiKey: this.openaiApiKey });
