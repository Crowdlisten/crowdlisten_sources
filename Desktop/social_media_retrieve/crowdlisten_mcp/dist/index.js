...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        throw new McpError(ErrorCode.InternalError, `Error executing ${name}: ${error.message || 'Unknown error'}`);
    }
});
// Tool handlers
async function handleGetTrendingContent(args) {
    ...
    if (platform === 'all') {
        ...
        const posts = await unifiedService.getTrendingContent(platform, limit);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
...
async function handleGetUserContent(args) {
    const { platform, userId, limit = 10 } = args;
    const posts = await unifiedService.getUserContent(platform, userId, limit);
    return {
        content: [{
                type: 'text',
                text: JSON.stringify({
...
async function handleSearchContent(args) {
    ...
    if (platform === 'all') {
        ...
        const posts = await unifiedService.searchContent(platform, query, limit);
        return {
            content: [{
                    type: 'text',
                    text: JSON.stringify({
...
async function handleGetContentComments(args) {
    const { platform, contentId, limit = 20 } = args;
    const comments = await unifiedService.getContentComments(platform, contentId, limit);
    return {
        content: [{
                type: 'text',
                text: JSON.stringify({
...
async function handleAnalyzeContent(args) {
    ...
    const analysis = await unifiedService.analyzeContent(platform, contentId, enableClustering);
    return {
        content: [{
                type: 'text',
