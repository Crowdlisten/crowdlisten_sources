export class CommentClusteringService {
     ...
     */
    selectCommentsForAnalysis(comments, maxComments, logs) {
     ...
     */
    calculateEngagementWeights(comments, logs) {
        ...
        const normalizedWeights = weights.map(weight => {
            if (range === 0)
                return 1.0; // All comments have same engagement
     ...
     */
    calculateSingleEngagementScore(comment) {
        let score = 0;
     ...
     */
    async performWeightedClustering(comments, embeddings, weights, logs) {
        ...
        // Assign comments to clusters based on weighted similarity
        weightedComments.forEach(({ comment, embedding, weight, index }) => {
            let bestCluster = 0;
            let bestScore = -Infinity;
            for (let clusterIdx = 0; clusterIdx < nClusters; clusterIdx++) {
                // Calculate weighted similarity score
                // Higher engagement comments have more influence on cluster assignment
                const baseScore = Math.random(); // Simplified for demo - would use actual similarity
     ...
     * Determine optimal number of clusters based on engagement distribution
     */
    determineOptimalClusters(comments, weights, logs) {
     ...
     */
    async analyzeClusterThemes(clusters, weights, logs) {
        try {
            // Import OpenAI dynamically
            const { OpenAI } = await import('openai');
            const openai = new OpenAI({ apiKey: this.openaiApiKey });
