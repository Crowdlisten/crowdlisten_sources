...
export class UnifiedSocialMediaService {
     ...
     */
    async getAllUserContent(userId, limit) {
        const results = {};
        const limitPerPlatform = limit ? Math.ceil(limit / this.adapters.size) : 10;
        const promises = Array.from(this.adapters.entries()).map(async ([platform, adapter]) => {
            try {
     ...
     */
    async analyzeContent(platform, contentId, enableClustering = true) {
        const adapter = this.getAdapter(platform);
    ...
    getAdapter(platform) {
        if (!this.isInitialized) {
            throw new SocialMediaError('Service not initialized', 'NOT_INITIALIZED', 'tiktok' // Default platform for service-level errors
            );
        }
